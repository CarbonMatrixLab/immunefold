...
for k, v in model_state_dict.items():
        ...
        if k in lora_keys:
            ...
            new_model_state_dict[k] = model_state_dict[k] + torch.matmul(model_state_dict[k_lora_B], model_state_dict[k_lora_A])
        else:
            new_model_state_dict[k] = v

out_file = in_file.split('.ckpt')[0] + '_lora_merged.ckpt'

torch.save(dict(model_state_dict=model_state_dict), out_file)