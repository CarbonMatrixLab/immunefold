...
torchrun \
    ...
    --master_addr="${host[1]}" \
    --master_port="29501" \
    ./train.py --config-name=train_ab  >>  train_rank0_${SLURM_JOB_ID}.log 2>&1 &
...
do
	...
	srun -N 1 --gres=gpu:4 -w ${host[${r}]} \
        ...
        --master_addr="${host[1]}" \
        --master_port="29501" \
        ./train.py --config-name=train_ab >>  train_rank${rr}_${SLURM_JOB_ID}.log 2>&1 &
done
